    ...
    <div style="" class="card card-body shadow-xl mt-n12 mx-3 mx-md-4">
        ...
        <div class="container">
            <div class="section text-left my-4">
                @php
                    if (isset($bookRatings)) {
                        $nbooks = count($bookRatings);
                } @endphp
                @if ($nbooks > 0)
                    <div class="card">
                                ...
                                <tbody>
                                    @foreach ($bookRatings as $book)
                                        <tr>
                                            ...
                                            <td class="align-middle text-center">{{ $book->user->email }}</td>
                                            <td class="align-middle text-center">{{ $book->book->book_title }}</td>
                                            <td class="align-middle text-center">{{ $book->rating }}</td>
                                            <td class="align-middle text-center">{{ $book->comment }}</td>
                                            <td class="align-middle text-center">{{ $book->created_at }}</td>
                                            <td class="align-middle text-center">{{ $book->updated_at }}</td>
                                            <td class="align-middle text-center">
                                                <form action="{{ route('rating.book.destroy', $book->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
